...
a = 0.01
b = 1
β0 = 1  # High temperature warm-up  # 3 for d = 256
...
if l_AIS:
    β = 3  # Low temperature # 6 for d = 256
    N_AIS = 100 
...
T = 20
scale = 25 / β0 # 80 for d = 16 & 120 for d = 64 & 50 for d = 256
dt = 0.0005 * scale
N = int(scale/dt)  
...
def V(U, AIS_step=0):
    ...
    return V_grid[AIS_step] * β * ( a * np.sum((U - np.roll(U, 1, axis = 1))**2, axis = 1) + b * np.sum(((1 - U**2)**2)[:, 1:-1], axis = 1) )
        
def V_grad(U, AIS_step=0):
    
    U = padding(U)
    
    return V_grid[AIS_step] * β * ( a * (2*U - np.roll(U, 1, axis = 1) - np.roll(U, -1, axis = 1)) - b * 4 * U * (1 - U**2) ) [:, 1:-1]
