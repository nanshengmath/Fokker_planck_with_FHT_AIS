...
λ = h / 8
a = - 0.01
β0 = 20  # High temperature warm-up 
...
if l_AIS:
    β = 32  # Low temperature # 6 for d = 256
    N_AIS = 100 
...
if l_AIS:
    ...
    data_path = f"./Saved_Data/2D_GZ_data_d_{d}_MALA"
...
def V(U, AIS_step=0):
    ...
    ) + h**2 * np.sum(((1 - U**2)**2 + a * U**3), axis = (1,2)) /4/λ )
...
def V_grad(U, AIS_step=0):
    ...
    ) - h**2 * (U * (1 - U**2) - 3*a/4 * U**2) /λ ).reshape((U.shape[0], -1))
...
for T_count in range(1,T+1):
    ...
    local_samples_β0 = np.empty((0, n, n))
    if l_AIS:
    ...
    for iteration in range(N_BATCH_NODE):
        ...
        local_samples_β0 = np.concatenate((local_samples_β0, padding(U)), axis=0)
        ...
        if l_AIS:
            ...
            local_samples = np.concatenate((local_samples, padding(U_new)), axis=0)
...
if rank == 0:
    ...
    if l_AIS:
        ...
        samples_1_mar = all_samples[:, round(n/2), round(n/2)]  # Extract the (n/2,n/2)-th bit from each sample
        plt.hist(samples_1_mar, bins=50, density=True)
        plt.title(f"{round(n/2),round(n/2)}-th Marginal Distribution")
